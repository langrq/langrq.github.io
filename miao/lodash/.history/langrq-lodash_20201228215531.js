var langrq = function () {
    ...
    //     if (Array.isArray(array)) {
    //         for (var key of array) {
    //             result.push(predicate(key, indexOf(item), array))
    //         }
    ...
    function map(collection, iteratee) {
        ...
        let iterFun = iteratee(iteratee)

        if (isObjectButNull(collection)) {
            for (let key in collection) {
                res.push(iterFun(collection[key]))
            }
        }
        if (Array.isArray(collection)) {
            for (let i = 0; i < collection.length; i++) {
                res.push(iterFun(collection[i], i, collection))
            }
        }
        return res
    }

    function targFunc(iteratee) {
        if (!iteratee) {
            return identity // return it => it
        }
        ...
        else if (isObjectButNull(iteratee)) {
