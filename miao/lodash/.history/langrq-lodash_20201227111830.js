var langrq = function () {
    ...
    function flatMapDeep(array, predicate) {
        ...
        for (var i = 0; i < array.length; i++) {
            ...
            if (typeof ary == 'object') {
                flatMapDeep(ary, predicate)          //递归内部数组
            } else {
                return result = flatMap(array, predicate)
            }
        }
    }
    ...
    function flatMapDepth(array, predicate, depth = 1) {
        ...
        for (var i = 0; i < array.length; i++) {
            var result = array[i]
            if (typeof result == 'object' && depth) {
                depth--
                flatMapDeep(result, predicate)   //递归内部数组
            } else {
                result = flatMap(result, predicate)
            }
        }
        return result
    }
