var langrq = function () {
    ...
    function iteratee() {
        ...
        if (Array.isArray(predicate)) {
            return mathchesProperty(predicate)
        }
    ...
    function differenceBy(array, valus, f = null) {
        var
    }

    function differenceWith(array, valus, comparator) {                    //此方法类似于_.difference，不同之处在于它接受比较器，该比较器被调用以将数组的元素与值进行比较。 结果值的顺序和参考由第一个数组确定。 比较器由两个参数调用：（arrVal，othVal）。
        var result = []
    ...
    function dropRightWhile(ary, predicate = null) {            //创建一个数组切片，其中不包括从末尾放置的元素。 元素将被删除，直到谓词返回false。 谓词由三个参数调用：（值，索引，数组）。
        ...
        if (Array.isArray(predicate)) {
            predicate = _.matchesProperty(predicate)
        }
        if (typeof predicate === 'object') {
    ...
    function matchesProperty(path, srcValue) {              //创建一个函数，该函数执行给定对象路径到srcValue的值之间的部分深度比较，如果对象值相等，则返回true，否则返回false。
        return function (obj) {
            return path = srcValue
        }
    }
    ...
    function matches(src) {                                //isMathch的绑定第二个参数的方式就是matches
        return bind(isMathch, null, window, src)
    }
    ...
    function property(path) {                 //创建一个函数，该函数在给定对象的路径上返回值。 //高阶函数
        var names = path.split('.')
        return function (obj) {
