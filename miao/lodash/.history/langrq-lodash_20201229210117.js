var langrq = function () {
    ...
    function matchesProperty(path, srcVal) {
        path = toPath(path)
        return item => {
            return get(item, path) === srcVal
        }
    }
    ...
    function toPath(val) {
        return val.split(/\.|\[|\]/g).filter(it => it.length)
    }
    ...
    function partition(collection, predicate) {
        ...
        collection.filter(it => {
            if (f(it)) left.push(it)
            if (!f(it)) right.push(it)
        })
        return left.concat(right)
    }
