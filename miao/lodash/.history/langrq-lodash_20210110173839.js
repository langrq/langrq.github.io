var langrq = function () {
    ...
    function isNaN(value) {
        if (typeof value == "object") return value = value.valueof()
        return value != value
    }
    ...
    function invoke(obj, path, ...args) {
        ...
        funcs = []
        while (isFunction(path[path.length - 1])) {
            funcs.push(func = path.slice(0, paht.length))
        }
        var result = has(obj, path)
        for (var key of funcs) {
            result = result[key]
        }
        return result
    }
